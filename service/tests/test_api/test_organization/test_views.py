...
class TestOrganizationsApi:
    ...
    def test_create_new_organization(self, api_client, organization_factory):
        assert api_client.post(
            '/v1/organizations/',
            data=factory.build(dict, FACTORY_CLASS=organization_factory)
        ).status_code == status.HTTP_201_CREATED

    def test_update_organization(self, api_client, organization, organization_factory):
